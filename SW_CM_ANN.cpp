// SW_CM_ANN.cpp : Этот файл содержит функцию "main". Здесь начинается и заканчивается выполнение программы.
//
...
double FILES(double times, int length, vector<double>& vectorVEL, vector<double>& vectorPRESS, vector<double>& vectorDENS, vector<double>& vectorENERG, vector<double>& Z)
{
	...
	EXIT.open("CU_CUBE_MSS_" + PARAMETR.direct_string + "_" + PARAMETR.vel_string + "_POR006_t" + tempSTRING + ".txt");
...
int main()
{
	...
	if (PAR_MSS.metall == 3)
	{
		...
		GG = 49.79957143e9 - 2.150571429e7 * PAR_MSS.TEMPERATURE;//CU
		//KK = 79.9e9 + 1.9e7 * PAR_MSS.TEMPERATURE - 3.7e4 * pow(PAR_MSS.TEMPERATURE, 2); //AL
		KK = 145.5314643e9 - 1.24547619e7 * PAR_MSS.TEMPERATURE - 1.963690476e4 * pow(PAR_MSS.TEMPERATURE, 2); //CU
		//Rho = 8940;
		Rho = 8.890885714e3 - 3.575e-1 * PAR_MSS.TEMPERATURE - 1.192857143e-4 * pow(PAR_MSS.TEMPERATURE, 2);
	}
	...
	while (TIME < PAR_MSS.timesss)
	{
		...
		for (int i = 0; i < PAR_MSS.length; i++)
		{
			...
			//вычисление ИНС и получение выходного вектора
			ANN.FORWARD_PROPAGATION(input, output);// *1e9;

			PRESS1[i] = (output[0] * 1e9) * (1 / (1 - output[4]));// +(output[0] * PAR_MSS.correct_press) * 1e9;// -PAR_MSS.correct_press;
		...
		if (TIME > 1.00e-7 && TIME < 1.011e-7)
		{
			double tttt = TIME;
			FILES(tttt, PAR_MSS.length, VELOCITY1, PRESS1, DENSITY1, ENERGY1, Z);
			//GNUPLOT(VELOCITY1, PRESS1, DENSITY1, ENERGY1, Z);
		}
		if (TIME > 2.00e-7 && TIME < 2.011e-7)
		{
			double tttt = TIME;
			FILES(tttt, PAR_MSS.length, VELOCITY1, PRESS1, DENSITY1, ENERGY1, Z);
			//GNUPLOT(VELOCITY1, PRESS1, DENSITY1, ENERGY1, Z);
		}
		if (TIME > 3.00e-7 && TIME < 3.011e-7)
		{
			double tttt = TIME;
			FILES(tttt, PAR_MSS.length, VELOCITY1, PRESS1, DENSITY1, ENERGY1, Z);
			//GNUPLOT(VELOCITY1, PRESS1, DENSITY1, ENERGY1, Z);
		}
		if (TIME > 4.00e-7 && TIME < 4.011e-7)
		{
			double tttt = TIME;
			FILES(tttt, PAR_MSS.length, VELOCITY1, PRESS1, DENSITY1, ENERGY1, Z);
			//GNUPLOT(VELOCITY1, PRESS1, DENSITY1, ENERGY1, Z);
		}
		if (TIME > 5.00e-7 && TIME < 5.011e-7)
		{
			double tttt = TIME;
			FILES(tttt, PAR_MSS.length, VELOCITY1, PRESS1, DENSITY1, ENERGY1, Z);
			//GNUPLOT(VELOCITY1, PRESS1, DENSITY1, ENERGY1, Z);
		}
		if (TIME > 6.00e-7 && TIME < 6.011e-7)
		{
			double tttt = TIME;
			FILES(tttt, PAR_MSS.length, VELOCITY1, PRESS1, DENSITY1, ENERGY1, Z);
			//GNUPLOT(VELOCITY1, PRESS1, DENSITY1, ENERGY1, Z);
		}
		if (TIME > 7.00e-7 && TIME < 7.011e-7)
		{
			double tttt = TIME;
			FILES(tttt, PAR_MSS.length, VELOCITY1, PRESS1, DENSITY1, ENERGY1, Z);
			//GNUPLOT(VELOCITY1, PRESS1, DENSITY1, ENERGY1, Z);
		...
		if (TIME > 1.00e-6 && TIME < 1.001e-6)
		{
			double tttt = TIME;
			FILES(tttt, PAR_MSS.length, VELOCITY1, PRESS1, DENSITY1, ENERGY1, Z);
			//GNUPLOT(VELOCITY1, PRESS1, DENSITY1, ENERGY1, Z);
		}
		if (TIME > 1.5e-6 && TIME < 1.501e-6)
		{
			double tttt = TIME;
			FILES(tttt, PAR_MSS.length, VELOCITY1, PRESS1, DENSITY1, ENERGY1, Z);
			//GNUPLOT(VELOCITY1, PRESS1, DENSITY1, ENERGY1, Z);
		}
		if (TIME > 2.00e-6 && TIME < 2.001e-6)
		{
			double tttt = TIME;
			FILES(tttt, PAR_MSS.length, VELOCITY1, PRESS1, DENSITY1, ENERGY1, Z);
			//GNUPLOT(VELOCITY1, PRESS1, DENSITY1, ENERGY1, Z);
		...
		if (TIME > 4.00e-6 && TIME < 4.011e-6)
		{
			double tttt = TIME;
			FILES(tttt, PAR_MSS.length, VELOCITY1, PRESS1, DENSITY1, ENERGY1, Z);
			//GNUPLOT(VELOCITY1, PRESS1, DENSITY1, ENERGY1, Z);
		}
		if (TIME > 4.49e-6 && TIME < 4.501e-6)
		{
			double tttt = TIME;
			FILES(tttt, PAR_MSS.length, VELOCITY1, PRESS1, DENSITY1, ENERGY1, Z);
			//GNUPLOT(VELOCITY1, PRESS1, DENSITY1, ENERGY1, Z);
		}
		if (TIME > 4.99e-6 && TIME < 5.002e-6)
		{
			double tttt = TIME;
			FILES(tttt, PAR_MSS.length, VELOCITY1, PRESS1, DENSITY1, ENERGY1, Z);
			//GNUPLOT(VELOCITY1, PRESS1, DENSITY1, ENERGY1, Z);
